...
module.exports = async (req, res) => {
  ...
  const result = await thirdCategoryManager.update(req.params.id, req.body)
  if (result) {
...
/**
 * @openapi
 * /third-categories/update/{id}:
 *   patch:
 *     tags:
 *       - Third Category
 *     description: Actualiza los campos de una categoría de tercer nivel, como el nombre o el estado activo.
 *     security:
 *       - BearerAuth: []
 *     parameters:
 *       - name: id
 *         in: path
 *         description: ID de la categoría de tercer nivel que se desea actualizar.
 *         required: true
 *         schema:
 *           type: integer
 *     requestBody:
