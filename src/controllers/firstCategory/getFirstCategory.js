...
module.exports = async (req, res) => {
  ...
  const result = await firstCategoryManager.getById(req.params.id)
  if (result) {
 ...
 *     tags:
 *       - First Category
 *     description: Devuelve los detalles de una categoría de primer nivel a partir de su ID.
 *     security:
 *       - BearerAuth: []
 *     parameters:
 *       - name: id
 *         in: path
 *         description: ID de la categoría de primer nivel que se desea obtener.
 *         required: true
 *         schema:
 *           type: integer
 *     responses:
