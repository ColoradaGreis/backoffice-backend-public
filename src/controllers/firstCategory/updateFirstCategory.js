...
module.exports = async (req, res) => {
  ...
  const result = await firstCategoryManager.update(req.params.id, req.body)
  if (result) {
...
/**
 * @openapi
 * /first-categories/update/{id}:
 *   patch:
 *     tags:
 *       - First Category
 *     description: Actualiza los campos de una categoría de primer nivel, como el nombre o el estado activo.
 *     security:
 *       - BearerAuth: []
 *     parameters:
 *       - name: id
 *         in: path
 *         description: ID de la categoría de primer nivel que se desea actualizar.
 *         required: true
 *         schema:
 *           type: integer
 *     requestBody:
