...
module.exports = async (req, res) => {
  ...
  const result = await secondCategoryManager.getById(req.params.id)
  if (result) {
 ...
 *     tags:
 *       - Second Category
 *     description: Devuelve los detalles de una categoría de segundo nivel a partir de su ID.
 *     security:
 *       - BearerAuth: []
 *     parameters:
 *       - name: id
 *         in: path
 *         description: ID de la categoría de segundo nivel que se desea obtener.
 *         required: true
 *         schema:
 *           type: integer
 *     responses:
